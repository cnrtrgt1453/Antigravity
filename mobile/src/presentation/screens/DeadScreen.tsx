...
export const DeadScreen: React.FC = () => {
      ...
      <View style={styles.card}>
        ...
        <View style={styles.infoRow}>
          ...
          <View>
            ...
            <Text style={[styles.value, { color: '#F85149' }]}>{diff}</Text>
          </View>
        </View>
      </View>
    );
  ...
  if (loading) {
    return <ActivityIndicator style={styles.loader} color="#DA3633" />;
  }

  return (
    <View style={styles.container}>
      <Text style={styles.title}>Dead Cross (7 Gün)</Text>
      <FlatList
        data={data}
        renderItem={renderItem}
        keyExtractor={(item) => item.ticker}
        contentContainerStyle={styles.list}
        refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor="#DA3633" />}
        ListEmptyComponent={<Text style={styles.emptyText}>Son 7 günde Dead Cross sinyali bulunamadı.</Text>}
      />
    </View>
  );
};
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 24, fontWeight: '800', color: '#DA3633', marginBottom: 20 },
